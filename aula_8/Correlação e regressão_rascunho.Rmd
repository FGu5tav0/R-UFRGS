...
```{r}
# aplica a função shapiro.test nas colunas (2) dados 
apply() # < insira o código 
```
...
# Removendo valores não significativos e alterando parâmetros estéticos 
# < insira o código 
```
...
```{r}
#modelo linear Prod = NPK . X
mod<-lm()  # < insira o código 
summary(mod)
anova(mod)
#Análise gráfica 
plot(mod)
...
```{r}
#Dados 
dadosn<-readxl::read_xlsx("seu/caminho/aqui/N_remanescente.xlsx")
...
#linear
linear_plateau() # < insira o código 

#quadratica
quadratic_plateau() # < insira o código  

# mitscherlich
mitscherlich() # < insira o código  

# Arcsine-Log
mod_alcc() # < insira o código  

# Cate and Nelson (1965)
cate_nelson_1971() # < insira o código  
