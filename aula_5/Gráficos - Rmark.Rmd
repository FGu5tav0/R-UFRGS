...
```{r}
data(bachmaier.nitrogen) # visualizar os dados 
dados<-data.frame(bachmaier.nitrogen) # trocando o nome do data frame
...
```{r}
ggsave(filename = "/Users/gustavopesini/Library/CloudStorage/OneDrive-Personal/ponto.png", plot = p1, width = 3, height = 2, dpi = 500, )
```
...
ggplot(dados, aes(zone_pt, yield))+
  ...
  scale_color_viridis_b(name = "Dose de N")+
   xlab(expression("Zona de fertilidade"))+ 
  ylab(expression("Produtividade de trigo (Mg ha"^"-1"~")"))
```
